...
請到以下連結下載CBOE的[Volatility Settlements資料]()。
...
請用以下連結下載某一場中華職棒的比賽[文字記錄]()。裡面的內容分別是每一個半局每個打擊者的記錄。記錄的內容有中文有英文，甚至有標點符號。請你將英數符號去掉，只留下中文字，並把這些中文字整理成一個字頻表data frame。也就是每個字使用了多少次，並且按照頻率從高到低排列。最後請將字頻data frame做圖表示。
